        ...
        if os.path.isdir(path):
            ...
            for fname in sorted(os.listdir(path)):
                ...
                with open(fpath, 'r', encoding="utf-16") as data_file:
                    d['text'] = process_data(str(data_file.read()))
...
# remove digit
# remove non-words charater
def process_data(news, stopwords):

	# replace encode charater into 1 format
    process = UniStd(news)

    # print(process)

    # replace non-words charaters
    ...
    for stw in stopwords:
    	...
    	text = regex.sub(lambda m: m.group().replace(stw," ",1), text)
...
with open('../data/News/Test_Full/Phap luat/PL_NLD_T_ (3882).txt', 'r', encoding="utf-16") as data_file:
	text = process_data(data_file.read().strip(), stopwords)
	print(text)